...
const resolve_hostname = function(host_name, callback) {
  ...
  var lookup = cp.spawn(DNS_SD, ["-G ", "v4", host_name], opts);
...
const resolve_service = function(task, callback) {
  ...
  var lookup = cp.spawn(DNS_SD, ["-L ", service_name, service_type, domain], opts);
...
class MDNS_Spawn extends Events.EventEmitter {
  ...
  start() {
    ...
    this._proc = cp.spawn(DNS_SD, ["-B", this._service_type, this._domain], opts);
